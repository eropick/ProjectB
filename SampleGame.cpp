...
SampleGame::SampleGame(int width, int height, int fpsLimit,int option)
	: BaseGame(width, height, fpsLimit,option), isSelectBall(false), playerBall(nullptr), isDraggedMouse(false)
{
	//게임 로그 남기도록
	system("cls");
...
void SampleGame::handle(sf::Event& ev)
{
	...
	case sf::Event::MouseMoved:
		...
		if (Player::WhoisTurn().isWin() != WIN) {
			...
			if (isDraggedMouse == true&&SampleBilliardBoard::inBoard(mouseXY)){
				Posvec = Posvec + (mouseXY - Posvec);
			}
		}
		break;
	...
	case sf::Event::MouseButtonReleased:
		...
		else if (ev.mouseButton.button == sf::Mouse::Left && power->isDraggingPower()){ //파워 뗐을 때
